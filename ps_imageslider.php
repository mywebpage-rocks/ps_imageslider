...
class Ps_ImageSlider extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->description = $this->trans('Zadanie 1', [], 'Modules.Imageslider.Admin');
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 3; ++$i) {
            ...
            $slide->position = $i;
            $slide->active = 1;
            $slide->is_mobile = ($i % 2); //mark every 2nd slide mobile
    ...
    protected function createTables()
    {
        ...
        ');
        $res &= Db::getInstance()->execute('
        ALTER TABLE `' . _DB_PREFIX_ . 'homeslider_slides` ADD COLUMN IF NOT EXISTS `is_mobile` tinyint(1) unsigned NOT NULL DEFAULT \'0\';
    ');
    ...
    public function getContent()
    {
        ...
        /* Validate & process */
        if (
            Tools::isSubmit('submitSlide') || Tools::isSubmit('submitSlideMobile') ||
            Tools::isSubmit('delete_id_slide') ||
            Tools::isSubmit('submitSlider') ||
            Tools::isSubmit('changeStatus') || Tools::isSubmit('changeDevice')
        ) {
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
        ...
        } elseif (Tools::isSubmit('changeDevice') && Tools::isSubmit('id_slide')) {
            $slide = new Ps_HomeSlide((int) Tools::getValue('id_slide'));
            if ($slide->is_mobile == 0) {
                $slide->is_mobile = 1;
            } else {
                $slide->is_mobile = 0;
            }
            ...
            $this->clearCache();
            $this->_html .= ($res ? $this->displayConfirmation($this->trans('Configuration updated', [], 'Admin.Notifications.Success')) : $this->displayError($this->getTranslator()->trans('The configuration could not be updated.', [], 'Modules.Imageslider.Admin')));
        } elseif (Tools::isSubmit('submitSlide') || (int) Tools::isSubmit('submitSlideMobile')) {
            /* Sets ID if needed */
            ...
            $slide->is_mobile = (int) Tools::getValue('is_mobile');
    ...
    {
        $device = $this->context->getDevice(); // 1 - desktop
        $slides_all = $this->getSlides(true, $device);
        $slides = [];
        if (!empty($slides_all)) {
            foreach ($slides_all as $slide) {
                if ($device == 1 && !$slide['is_mobile'] || $device != 1 && $slide['is_mobile']) {
                    $slides[] = $slide;
                }
            }
        }
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance((bool) _PS_USE_SQL_SLAVE_)->executeS(
            'SELECT hs.`id_homeslider_slides` as id_slide, hss.`position`, hss.`is_mobile`, hss.`active`, hssl.`title`,
    ...
    public function displayIsMobile($id_slide, $device)
    {
        $title = ((int) $device == 0 ? $this->trans('Desktop', [], 'Admin.Global') : $this->trans('Mobile', [], 'Admin.Global'));
        $icon = ((int) $device == 0 ? 'icon-desktop' : 'icon-mobile');
        $class = ((int) $device == 0 ? 'btn-info' : 'btn-info');
        $html = '<a class="btn ' . $class . '" href="' . AdminController::$currentIndex .
            '&configure=' . $this->name .
            '&token=' . Tools::getAdminTokenLite('AdminModules') .
            '&changeDevice&id_slide=' . (int) $id_slide . '" title="' . $title . '"><i class="' . $icon . '"></i> ' . $title . '</a>';
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            $slides[$key]['is_mobile'] = $this->displayIsMobile($slide['id_slide'], $slide['is_mobile']);
    ...
    public function renderAddForm()
    {
        $fields_form = [
            'form' => [
                'legend' => [
                    'title' => ($this->device == 'desktop') ? $this->trans('Slide information', [], 'Modules.Imageslider.Admin') : $this->trans('Mobile slide information', [], 'Modules.Imageslider.Admin'),
                    'icon' => 'icon-cogs',
                ],
                'input' => [
                    [
                        'type' => 'file_lang',
                        'label' => ($this->device == 'desktop') ? $this->trans('Image', [], 'Admin.Global') : $this->trans('Mobile Image', [], 'Admin.Global'),
                        'name' => 'image',
                        'required' => true,
                        'lang' => true,
                    ...
                    [
                        'type' => 'switch',
                        'label' => $this->trans('Device', [], 'Admin.Global'),
                        'name' => 'is_mobile',
                        'is_bool' => true,
                        'values' => [
                            [
                                'id' => 'active_on',
                                'value' => 1,
                                'label' => $this->trans('Mobile', [], 'Admin.Global'),
                            ],
                            [
                                'id' => 'active_off',
                                'value' => 0,
                                'label' => $this->trans('Desktop', [], 'Admin.Global'),
                            ],
                        ],
                    ],
                ],
        ...
        $helper->submit_action = $this->device == 'desktop' ? 'submitSlide' : 'submitSlideMobile';
    ...
    public function getAddFieldsValues()
    {
        ...
        $fields['is_mobile'] = Tools::getValue('is_mobile');
